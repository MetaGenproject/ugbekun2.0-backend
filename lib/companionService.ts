 ...
 */
export async function buildSystemPrompt(studentId: number, prisma: any): Promise<string> {
  try {
    ...
    if (enroll) {
      ...
      if (assigns.length > 0) {
        subjectList = assigns.map((a: any) => a.subject.name).join(', ');
      }
    }
    ...
    const allScores = [
      ...hwScores.map((s: any) => s.score),
      ...examScores.map((s: any) => s.totalMark),
    ];
    const avgScore =
      allScores.length > 0
