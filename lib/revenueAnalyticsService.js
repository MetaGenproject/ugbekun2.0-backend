...
async function getMultiBranchRevenueAnalytics(prisma, { sessionId, branchId, period = 'all' }) {
  const branchFilter = branchId && !isNaN(parseInt(branchId, 10)) ? { id: parseInt(branchId, 10) } : {}
  const sessionFilter = sessionId && !isNaN(parseInt(sessionId, 10)) ? { sessionId: parseInt(sessionId, 10) } : {}
  ...
  const invoiceWhere = {
    ...sessionFilter,
    ...(branchId && !isNaN(parseInt(branchId, 10)) ? { branchId: parseInt(branchId, 10) } : {})
  }
  ...
  branches.forEach((b) => {
    const activePaidSub = b.subscriptions.find(
      (s) => s.paymentStatus === 'paid' && new Date(s.expiryDate) >= now
    )
  ...
  const branchLeaderboard = Object.values(branchMetrics).map((b) => {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  const planRevenueBreakdown = subscriptionPlans.map((p) => {
    const rev = p.subscriptions.reduce((sum, s) => sum + parseFloat(s.totalCost.toString()), 0)
    return {
      name: p.name,
      slug: p.slug,
 ...
 */
function exportRevenueReportCsv(analyticsData) {
  const headers = [
    'Branch ID',
    'Branch Name',
    'Branch Code',
  ...
  const rows = analyticsData.branchLeaderboard.map((b) => [
    b.branchId,
 ...
 */
function exportRevenueReportPdf(analyticsData, title = 'Multi-Branch Superadmin Revenue Audit Report') {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Table Rows
      analyticsData.branchLeaderboard.forEach((branch, index) => {
        if (yPos > 740) {
          doc.addPage()
          yPos = 36
