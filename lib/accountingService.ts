 ...
 */
export async function generateInvoiceNumber(prisma: any, branchId: number): Promise<string> {
...
export async function generateInvoice(
  prisma: any,
  { studentId, termLabel, feeTypeIds, branchId, sessionId, dueDate }: GenerateInvoiceParams
) {
  ...
  const totalAmount = feeTypes.reduce((acc: number, curr: any) => acc + parseFloat(curr.amount.toString()), 0);
  ...
  return prisma.$transaction(async (tx: any) => {
    ...
    const itemsData = feeTypes.map((ft: any) => ({
      description: ft.name,
...
export async function recordPayment(
  prisma: any,
  { invoiceId, amount, method, reference, receivedBy, notes, branchId }: RecordPaymentParams
) {
...
export async function getFinancialOverview(
  prisma: any,
  { branchId, sessionId }: { branchId?: number; sessionId?: number }
) {
...
export async function exportFinancialReportPdf(
  prisma: any,
  { branchId, sessionId, schoolName }: { branchId?: number; sessionId?: number; schoolName: string }
): Promise<Buffer> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(16).text(schoolName.toUpperCase(), 55, 52, { width: 485, align: 'left' });
      doc.font('Helvetica').fontSize(10).fillColor('#e8f5e9').text(`CONSOLIDATED FINANCIAL REPORT • ACADEMIC SESSION`, 55, 76);
      ...
      doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(11).text(
        `NGN ${overview.summary.totalInvoiced.toLocaleString(undefined, { minimumFractionDigits: 2 })}`,
        50,
        yPos + 25
      );
      ...
      doc.fillColor('#15803d').font('Helvetica-Bold').fontSize(11).text(
        `NGN ${overview.summary.totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2 })}`,
        225,
        yPos + 25
      );
      ...
      doc.fillColor('#b91c1c').font('Helvetica-Bold').fontSize(11).text(
        `NGN ${overview.summary.totalOutstanding.toLocaleString(undefined, { minimumFractionDigits: 2 })}`,
        400,
        yPos + 25
      );
      ...
      doc.fillColor(darkColor).font('Helvetica').fontSize(9).text(`Overall Fee Collection Rate: `, 40, yPos, { continued: true }).font('Helvetica-Bold').text(`${overview.summary.collectionRate.toFixed(2)}%`);
      ...
      doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(8).text('INVOICE NO', 45, yPos + 6).text('STUDENT NAME', 130, yPos + 6).text('TOTAL INVOICED', 300, yPos + 6).text('TOTAL PAID', 380, yPos + 6).text('OUTSTANDING', 460, yPos + 6);
      ...
      invoices.forEach((inv: any, index: number) => {
        ...
        doc.text(inv.invoiceNo, 45, yPos + 5).text(name, 130, yPos + 5, { width: 160, ellipsis: true }).text(total, 300, yPos + 5).text(paid, 380, yPos + 5).fillColor(parseFloat(inv.balanceAmount.toString()) > 0 ? '#b91c1c' : darkColor).text(balance, 460, yPos + 5).fillColor(darkColor);
      ...
      for (let i = 0; i < pageCount; i++) {
        ...
        doc.fillColor('#64748b').font('Helvetica').fontSize(7.5).text('Ugbekun 2.0 Consolidated Financial Audit System', 40, 765).text(`Page ${i + 1} of ${pageCount}`, 40, 765, { align: 'right', width: 515 });
      }
