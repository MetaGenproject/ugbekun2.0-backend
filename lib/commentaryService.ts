...
export interface StudentAiCommentaryParams {
  ...
  marksBySubject?: Record<string, number | string>;
 ...
 */
export async function generateStudentAiCommentary(params: StudentAiCommentaryParams): Promise<string> {
  const {
    studentName = 'Student',
    gender = 'Student',
    averageScore = 75,
 ...
 */
export function generatePedagogicalCommentaryFallback(params: {
  studentName: string;
  averageScore: number;
  attendanceRate: number;
 ...
 */
export async function generateBatchClassCommentary(
  studentsList: any[],
  tone = 'constructive'
): Promise<any[]> {
  const results = [];
