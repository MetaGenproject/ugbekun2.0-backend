 ...
 */
const branchConfigs: Record<number, any> = {};
const branchGateLogs: Record<number, any[]> = {};
const branchBusFleets: Record<number, any[]> = {};
const branchStudentTransports: Record<number, any> = {};
...
export function getInitialFleet(branchCode = 'SCH'): FleetBus[] {
  return [
    {
      id: 'BUS-01',
...
export function getInitialGateLogs(): GateLogEntry[] {
  const now = new Date();
  return [
    {
      id: 'SCAN-801',
 ...
 */
export async function saveMyEduRideConfig(prisma: any, branchId: number | string, updateData: any) {
  const bId = parseInt(branchId as string, 10);
 ...
 */
export async function testMyEduRideConnection({ apiUrl, apiKey, branchCode }: { apiUrl?: string; apiKey?: string; branchCode?: string }) {
 ...
 */
export async function syncStudentsToMyEduRide(prisma: any, branchId: number | string) {
  const bId = parseInt(branchId as string, 10);
  ...
  const rosterPayload = students.map((st: any) => ({
 ...
 */
export async function getTransportOverview(prisma: any, branchId: number | string) {
  const bId = parseInt(branchId as string, 10);
  const config = await getMyEduRideConfig(prisma, bId);
 ...
 */
export async function getBusFleet(prisma: any, branchId: number | string) {
  const bId = parseInt(branchId as string, 10);
  const config = await getMyEduRideConfig(prisma, bId);
 ...
 */
export async function processGateScan(
  prisma: any,
  branchId: number | string,
  { code, direction = 'ENTRY', gateLocation = 'Main Front Turnstile Gate 1', verifiedBy = 'Turnstile Scanner' }: any
) {
  const bId = parseInt(branchId as string, 10);
  if (!branchGateLogs[bId]) {
    branchGateLogs[bId] = [];
 ...
 */
export async function updateStudentBoarding(
  prisma: any,
  branchId: number | string,
  { studentId, busId, status = 'BOARDED_MORNING' }: any
) {
  const bId = parseInt(branchId as string, 10);
  if (!branchBusFleets[bId]) {
 ...
 */
export function exportGateLogsCsv(logs: any[], schoolName = 'Ugbekun Schools'): string {
  const headers = [
    'Scan ID',
    'Person Name',
    'Role Type',
 ...
 */
export function exportGateLogsPdf(logs: any[], schoolName = 'Ugbekun International Academy'): Promise<Buffer> {
