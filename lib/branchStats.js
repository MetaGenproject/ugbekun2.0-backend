...
async function getBranchStats(prisma, branchId, options = {}) {
  ...
  const [students, parents, teachers, staff, classesCount, subjectsCount, admissionsCount, invoiceAgg, paymentAgg] = await Promise.all([
    ...
    prisma.invoice ? prisma.invoice.aggregate({
      where: { branchId },
      _sum: { netAmount: true, paidAmount: true, dueAmount: true }
    }).catch(() => null) : Promise.resolve(null),
    prisma.payment ? prisma.payment.aggregate({
