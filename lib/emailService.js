...
const { buildOnboardingEmail, buildTeacherOnboardingEmail, buildGracePeriodExtensionEmail } = require('./emailTemplates')
 ...
 *
 * @param {object} payload
 * @param {string} payload.adminEmail       - Recipient email
 * @param {string} payload.schoolName       - Branch / school name
 * @param {number|string} payload.days      - Number of days extended
 * @param {string} payload.newExpiryDate    - Pre-formatted date string
 * @param {string} [payload.reason]         - Reason for extension
 * @param {string} [payload.loginUrl]       - Portal URL
 * @returns {Promise<{ success: boolean, messageId?: string, error?: string }>}
 */
async function sendGracePeriodExtensionEmail(payload) {
