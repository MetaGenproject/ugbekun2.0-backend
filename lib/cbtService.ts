 ...
 */
export function parseAikenFormat(text?: string | null): ParsedQuestion[] {
 ...
 */
export function parseCsvFormat(csvText?: string | null): ParsedQuestion[] {
 ...
 */
export function parseJsonFormat(jsonText: any): ParsedQuestion[] {
  if (!jsonText) return [];
  try {
...
export interface GenerateAiCurriculumQuestionsParams {
  subjectName?: string;
  topic?: string;
  classLevel?: string;
 ...
 */
export function generateAiCurriculumQuestions({
  subjectName = 'General Studies',
  topic = 'Core Concepts',
  classLevel = 'Secondary',
  questionCount = 5,
  questionType = 'mcq',
}: GenerateAiCurriculumQuestionsParams): ParsedQuestion[] {
  const count = Math.min(Math.max(parseInt(String(questionCount), 10) || 5, 1), 20);
  const templates = [
    {
 ...
 */
export function autoGradeCbtSubmission({
  questions = [],
  studentAnswers = [],
