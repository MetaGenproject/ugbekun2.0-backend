 ...
 */
export async function processStudentAttritionRisk(studentId: number, prisma: any) {
  // 1. Calculate risk
    ...
    if (formTeacherId) {
      ...
      if (!existingAlert) {
        ...
        try {
          ...
          const notesText = commentaries.map((c: any) => c.remark).join('; ') || 'No specific behavioral remarks.';
            ...
            messages: [
              {
                role: 'system',
                content:
                  'You are an expert student intervention counselor. You formulate empathetic, constructive Parent Engagement Plans.',
              },
              {
                role: 'user',
...
3. School-Home Check-ins (Frequency of progress updates)`,
          ...
          parentPlan = completion.choices[0]?.message?.content || null;
          remediationSteps = [
