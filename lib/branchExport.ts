...
export function formatBranchRow(branch: BranchRowData) {
  return {
    id: branch.id,
    name: branch.name,
    code: branch.code || '',
...
export function branchesToCsv(branches: BranchRowData[]): string {
...
export function buildBranchesPdf(
  ...
  title = 'Ugbekun Branch Directory'
): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    branches.forEach((branch, index) => {
      ...
      doc.text(
        `Students: ${row.students}  ·  Parents: ${row.parents}  ·  Teachers: ${row.teachers}  ·  Staff: ${row.staff}`
      );
      if (row.address) {
